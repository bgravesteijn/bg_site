...
```{r}
pubs$position <- factor(0)
levels(pubs$position) <- c(1:1000, "last")
for(i in 1:nrow(pubs)){
  ...
  pubs$position[i] <- p
  if(p==length(x)) pubs$position[i] <- "last"
  }
}

pie(table(pubs$position, exclude = names(which(table(pubs$position)==0))), 
